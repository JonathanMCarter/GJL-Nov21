...
Shader "Custom/Proximity" {
    Properties {
        _MainTex ("Base (RGB)", 2D) = "clear" {} // Regular object texture 
        ...
        _OutlineColour ("Outline Colour", color) = (0.0,0.5,1.0,1.0) // Colour of the outline
    }
    SubShader {
        ...
        float4 frag(vertexOutput input) : COLOR 
        {
            ...
            if (dist < _VisibleDistance) {
                return tex2D(_MainTex, float2(input.tex.x,input.tex.y)); // Visible
            }
            else if (dist < _VisibleDistance + _OutlineWidth) {
                return _OutlineColour; // Edge of visible range
            }
            else {
                float4 tex = tex2D(_MainTex, float2(input.tex.x,input.tex.y)); // Outside visible range
                tex.a = 0;
                return tex;
            }
        }
